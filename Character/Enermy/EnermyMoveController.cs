...
/*敌人移动控制器*/
public class EnermyMoveController : MonoBehaviour {
        ...
        if (!GetComponent<DetectObjects> ().detected_player) //如果没有发现玩家
        {
            ...
            if (GetComponent<EnermyRotateController> ().delta_vector.sqrMagnitude != 0) //如果玩家与敌人之间距离不为0
            {
                GetComponent<Rigidbody> ().velocity =
                    (-GetComponent<EnermyRotateController> ().delta_vector.normalized) *
                    (back_speed / GetComponent<EnermyRotateController> ().delta_vector.sqrMagnitude); //向远离玩家方向移动，并且距离越远，速度越慢
            }
        }
    }
}